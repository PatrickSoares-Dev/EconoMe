    ...
    public class TransacoesController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetTransacoesByUsuario()
        {
            try
            {
                ...
                // Adicionar o nome da instituição bancária ao response
                var transacoesComInstituicao = transacoes.Select(t => new
                {
                    t.TransacaoID,
                    t.ContaID,
                    t.CategoriaID,
                    ...
                    Conta = new
                    {
                        ...
                        t.Conta.FirebaseUID,
                        t.Conta.NomeConta,
                        t.Conta.InstituicaoBancariaID,
                        t.Conta.Saldo,
                        t.Conta.DataCriacao,
                        t.Conta.DataAtualizacao,
                        NomeInstituicaoBancaria = t.Conta.InstituicaoBancaria.Nome // Adiciona o nome da instituição bancária
                    },
                    Categoria = new
                    {
        ...
        public async Task<IActionResult> CreateTransacao([FromBody] TransacaoCreateDTO transacaoDTO)
        {
            try
            {
                ...
                await _contaService.UpdateConta(conta);
        ...
        [HttpGet("categoria")]
        public async Task<IActionResult> GetTransacoesByCategoriaId([FromQuery] TransacaoCategoriaDTO dto)
        {
            try
            {
                var firebaseUID = GetFirebaseUID();
                var transacoes = await _transacaoService.GetTransacoesByCategoriaId(dto.CategoriaID, dto.ContaID);
                return Ok(new ApiResponse<IEnumerable<Transacao>>
                {
                    Status = 200,
                    Message = "Transações recuperadas com sucesso",
