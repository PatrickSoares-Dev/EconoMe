        ...
        public async Task<IActionResult> CreateConta([FromBody] ContaCreateDTO contaCreateDTO)
        {
            try
            {
                ...
                var conta = new Conta
                {
                    ...
                    Saldo = contaCreateDTO.Saldo ?? 0.00M // Atribuir 0 se o saldo não for especificado
                };

                var createdConta = await _contaService.CreateConta(conta);

                // Verificar se foi fornecido um saldo inicial
                if (contaCreateDTO.Saldo.HasValue && contaCreateDTO.Saldo.Value > 0)
                {
                    // Criar transação de saldo inicial
                    var transacaoInicial = new Transacao
                    {
                        ContaID = createdConta.ContaID,
                        CategoriaID = 18, // ID da categoria "Saldo Inicial"
                        Tipo = "Saldo Inicial",
                        Valor = contaCreateDTO.Saldo.Value,
                        Descricao = "Saldo inicial da conta",
                        DataTransacao = DateTime.UtcNow,
                        FirebaseUID = firebaseUID,
                        NomeCategoria = "Saldo Inicial"
                    };

                    await _transacaoService.CreateTransacao(transacaoInicial); // Método para criar transação
                }
